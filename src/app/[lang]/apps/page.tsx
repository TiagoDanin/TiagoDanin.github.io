...
export async function generateMetadata({ params }: PageProps<'/[lang]/apps'>): Promise<Metadata> {
  ...
  return {
    title: t(i18n)`Android Apps - Google Play`,
    description: t(
      i18n
    )`Android apps built by Tiago Danin and published on Google Play, games, productivity tools, and relaxation apps.`,
    keywords: ["android apps", "google play", "tiago danin apps", "mobile games", "jetpack compose"],
    alternates: {
      ...localeAlternates(locale, '/apps'),
      types: markdownAlternate('/apps'),
    },
    openGraph: {
      ...
      "application/ld+json": JSON.stringify([
        {
          ...
          "@type": "CollectionPage",
          "name": t(i18n)`Android Apps by Tiago Danin`,
          "description": t(i18n)`Android apps published on Google Play by Tiago Danin`,
          "url": pageUrl(locale, '/apps'),
          "author": {
            "@type": "Person",
            "name": "Tiago Danin",
...
export default async function AppsPage({ params }: PageProps<'/[lang]/apps'>) {
  const locale = resolveLocale((await params).lang);
    ...
    <div className="min-h-screen">
        ...
        <div className="container mx-auto max-w-5xl relative z-10">
          <div className="text-center space-y-6">
            ...
            <h1 className="text-4xl md:text-6xl font-bold tracking-tight text-foreground">
              <Trans>My Android Apps</Trans>
            </h1>
            <p className="text-xl text-muted-foreground max-w-2xl mx-auto">
              <Trans>Games, utilities, and tools I&apos;ve built and shipped to Google Play, each one solving a real problem or scratching a creative itch.</Trans>
            </p>
          </div>
        </div>
      </section>
                ...
                <CardContent className="flex flex-col flex-1">
                  ...
                  <div className="flex gap-3">
                    <Button asChild className="flex-1">
                      <Link href={localePath(locale, `/app/${app.slug}`)}>
                        <Trans>Learn More</Trans> <ArrowRight className="h-4 w-4 ml-2" />
                      </Link>
                    </Button>
                    {app.url && (
                      <Button variant="outline" size="icon" asChild>
                        <a href={app.url} target="_blank" rel="noopener noreferrer" aria-label={t(i18n)`View on Google Play`}>
      ...
      <section className="py-20 px-4 bg-muted/30">
        <div className="container mx-auto max-w-3xl text-center space-y-6">
          <h2 className="text-3xl font-bold"><Trans>Want to build something together?</Trans></h2>
          <p className="text-xl text-muted-foreground">
            <Trans>I take on mobile development projects, from idea to App Store launch.</Trans>
          </p>
          <Button size="lg" asChild>
            <Link href={localePath(locale, "/mobile")}>
              <Smartphone className="h-5 w-5 mr-2" />
              <Trans>View Mobile Services</Trans>
            </Link>
          </Button>
        </div>
      </section>
