...
- **`builtRoutes()` in `next-sitemap.config.cjs`** enumerates pages for the two places that need a list, by walking the exported tree: a directory under `dist/` holding an `index.html` is a page, and nothing else in `dist/` has one. It takes a locale prefix (`''` or `'/br'`), skips `_next`, the RSC dumps and the error pages, and drops anything whose HTML says `noindex`. `scripts/generateSitemaps.ts` imports it, and the config's `transform` with it, so both sitemaps weight a page the same way and announce the same hreflang set. It replaced `scripts/appRoutes.cjs`, which read the App Router tree and so could only ever see the static routes.
...
- The site has **five** sitemaps, all generated and git-ignored. `scripts/generateSitemaps.ts` writes `sitemap.xml` (the index), `sitemap-site-br.xml` (the `/br/**` pages), `sitemap-project-github.xml` (the `/project/github/[slug]` landing pages, **both languages**) and `sitemap-homepage-github.xml` (the GitHub Pages homepages served under the custom domain, normalized from each repo's `homepage` field). `next-sitemap` then writes `sitemap-site.xml` for the English pages; it has `generateIndexSitemap: false` so it never overwrites the index, and excludes `/project/github/*`, `/br/*` and `/en/*` so the lists stay disjoint.
- **The page lists come from `dist/`, so `yarn sitemap` needs a build first.** `yarn deploy` chains build -> sitemap -> build, which is what makes it correct; running `yarn sitemap` against a stale `dist/` writes a sitemap of the previous build's pages, and against no `dist/` it throws. The two site sitemaps are exact mirrors of each other by construction, and a sitemap can no longer list a URL that was never exported. Both used to enumerate `contents/` separately and disagree: after the `[lang]` migration the English file listed 63 URLs against 379 the week before, having dropped every post, talk, project, app, tag, skill, timeline event and blog page, while the Portuguese one listed posts and talks and no project, app, timeline or social page at all.
